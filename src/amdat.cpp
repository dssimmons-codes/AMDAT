...
{
  using amdat::cli::collect_version_info;
  using amdat::cli::print_one_line_banner;
  using amdat::cli::print_version_text;
  using amdat::cli::print_version_json;
  using amdat::cli::stdout_is_tty;
  using amdat::cli::env_no_banner;

  auto V = collect_version_info();
  ...
  for(int argii=0;argii<argc;argii++) // loop over tokens
  {
    if(argv[argii][0] == '-') // flags
    {
      if(argv[argii][1] == 'v')
      {
        print_version_text(V);
        exit(0);
      }
      else if(argv[argii][1] == 'j')
      {
        print_version_json(V);
        exit(0);
      }
      else if(argv[argii][1] == 'q') // quiet
      {
        quiet_flag = true;
      }
      ...
      else if(argv[argii][1] == 'h' ||
              std::string(argv[argii]) == "--h" ||
              std::string(argv[argii]) == "--help" ) // help flag
      {
        cout<<"\nAmorphous Molecular Dynamics Analysis Toolkit (AMDAT) v."<<V.semver;
        cout<<"\nBuild: "<<V.build_date;
        ...
        cout<<"-j \t\t Print version information in machine-readable output (json) and exits"<<endl;
        cout<<"-q \t\t Quiet (suppress startup banner)"<<endl;
