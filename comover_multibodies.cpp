/*Methods for Comover_Multibodies class - Identifies particles participating in stringlike cooperative rearrangements and converts them to multibodies*/
...
Comover_Multibodies::Comover_Multibodies():Dynamic_Cluster_Multibodies()
{
  threshold=0;
  n_atomtypes=1;
...
Comover_Multibodies::Comover_Multibodies(const Comover_Multibodies& copy):Dynamic_Cluster_Multibodies(copy)
{
  int typeii, type2ii;
...
Comover_Multibodies::~Comover_Multibodies()
{
  for(int typeii=0;typeii<n_atomtypes;typeii++)
    {
      delete [] sigmatrix[typeii];
...
Comover_Multibodies Comover_Multibodies::operator=(const Comover_Multibodies& copy)
{
  if(this!=&copy)
  {
    int typeii, type2ii;
...
Comover_Multibodies::Comover_Multibodies(System * syst, int tgap, float thresh, string sigmatrixname):Dynamic_Cluster_Multibodies(syst,tgap)
{
  system=syst;
  threshold=thresh;
...
void Comover_Multibodies::allocate_sig_matrix(string sig_file)
{
  Tokenize tokenize;
...
bool Comover_Multibodies::clustered_check(Trajectory* trajectory1, Trajectory* trajectory2, int thisii, int nextii)
{
  bool check;
  ...
  distance1=(trajectory1->show_coordinate(thisii)-trajectory2->show_coordinate(thisii)).length_unwrapped(system->size(thisii));
  distance2=(trajectory1->show_coordinate(nextii)-trajectory2->show_coordinate(nextii)).length_unwrapped(system->size(nextii));
  
  check= (distance1<(threshold*sigmatrix[trajtype1][trajtype2])&&distance2<(threshold*sigmatrix[trajtype1][trajtype2]));
  return check;
}



Coordinate Comover_Multibodies::get_imageoffset(Trajectory* trajectory1, Trajectory* trajectory2, int thisii, int nextii)
{
