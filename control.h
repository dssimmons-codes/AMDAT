...
void Control::run_analysis(Analysis_type analyzer, string setline, string filename)
{
     ...
     **/
     ...
     else if (command == "bin_list")
     {
	  ...
	  if (listnum !=-1)
	  {
		 ...
		 if(dynamic&&use_persistence)
		 {
		    ...
		    bin_analyzer.analyze(trajectories[listnum]); 							// performs analysis on intersection of bins given by <bin_list_ID> and trajectories given by <list_ID>
		 ...
		 else
		 {
		    ...
		    static_bin_analyzer.analyze(trajectories[listnum]); 							// performs analysis on intersection of bins given by <bin_list_ID> and trajectories given by <list_ID>
     ...
     {
     cout << "Error: Particles, etc. not suppored by this feature. Use lists or bin_lists please" << endl;
     exit(1);
