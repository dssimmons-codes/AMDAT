...
_\<max\_length\_scale\>_ determines the longest distance which will be decomposed into inverse space, if a distance of 0 is given, the full box size is used_. \<(optional) fullblock\>_is 0 if structure factor should only be calculated at the start of each block, -1 if it should be calculated at all times, and a positive integer if it is to be calculated at an offset time index once per block.
